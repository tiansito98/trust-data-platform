...
- **This repo is PUBLIC.** Never commit personal data of identifiable people (employee names, commissions, salaries), exports with internal figures, Sixt documents covered by the datashare agreement, or customer data. All of that goes in **`docs/private/`**, which is gitignored (see `docs/private/README.md`). When in doubt, put it there.
...
| `gold_carro_dia` | 1 row / placa × día | **Capa gold.** Ocupación ácida (rentado vs flota), revenue/RPD por ciudad, prorrateado 24h, desde 2024. **Flota = roster activo de `bronze.fleet_shop_ve_fct_vehicles_current`** (defleeted afuera). **Sede por UBICACIÓN REAL día a día** via timeline de contratos: los **traslados** (`rate_type_level3_aknm='Internal Products'`) mueven el carro; un carro que se traslada a mitad de mes reparte sus días entre sedes. Flota por sede = flota-días/días (promedio; las placas distintas doble-cuentan traspasos). **GRANO SEGMENTO (v4, 2026-08-30):** los días rentados y el revenue se atribuyen a la **placa real de cada tramo** (`silver.fact_rental_vehicles`, contrato × `rvnc_hser`), NO a la placa del header — un contrato puede cambiar de carro a mitad de renta y antes (v3) el carro inicial se llevaba todos los días. Revenue por día = neto/N con N = suma de días 24h de todos los segmentos. Requiere el fix de PK de `ra_fct_rental_vehicles` (incluye `rvnc_hser`, ver hard rule 16). USD + COP (TRM Banrep del día de entrega). **`tar_usd`/`tar_cop` = tarifa pura (cargo `T`, lado RENTAL_COUNTER) prorrateada por día** (desde 2026-09-21; ~73% del neto) — alimenta el **Presupuesto** (rentas de carro sin adicionales/coberturas/tax); `adi_usd`/`adi_cop` siguen en 0. Construida por `build_gold_carro_dia` (SQL v6). Alimenta `9_Analitica` y `10_Presupuesto`. |
| `gold_cargo_dia` | 1 row / fecha × sede × código | **Capa gold.** Cargos por código prorrateados 24h+gracia (todos los períodos = ingreso completo). **Solo `fuente_cargo='RENTAL_COUNTER'`** (fix 2026-09-02): `vw_rentals_detail` es un UNION ra+rs; sin ese filtro el desglose en USD se duplicaba ~2× para contratos prepagados (el arm `RESERVA_ONLINE` trae el cargo completo en USD pero COP~0, por eso solo se veía en dólares). Cierre Diario y Cargos Granular ya filtraban así; gold_cargo_dia se lo saltaba. Ahora el desglose cuadra con el KPI de revenue dentro del ~2% (bruto por código vs neto). USD + COP (TRM del día de entrega). Alimenta el desglose de `9_Analitica`. |
...
13. **Comisiones de asesor:** 5% sobre el cargo **con IVA** (= 5,95% del valor sin IVA), en COP a TRM Banrep del día de entrega, solo sobre la porción **counter** de cargos *pure counter* (prepagado = 0) y solo para códigos en `COMISIONABLES`. **OT y FI NO comisionan** (decisión de negocio 2026-08-26). Se atribuye a `operador_handover_codigo`. **PRORRATEO POR DÍA (desde 2026-08-27):** la base comisionable se reparte por día efectivo 24h del contrato en la tabla `silver.fact_comision_dia` (1 fila/contrato×día, `base_comision_cop_civa_dia`). Una renta que cruza meses aporta a cada mes SOLO sus días (N = `CEIL((devolución−entrega − 1h)/24h)`, mín 1, sobre la duración COMPLETA — real si cerró, programada si abierta; días = entrega+0..N−1). **El día es un bloque de 24h CON 1 HORA DE GRACIA (desde 2026-09-01):** el sistema no cobra día extra si se devuelve dentro de la hora siguiente al bloque, así que 25h = 1 día. Aplica a las 3 tablas con regla de 24h (`fact_comision_dia`, `gold_carro_dia`, `gold_cargo_dia`). Cargos Granular filtra por ventana `[desde,hasta]` sobre `fecha`, así que un asesor sede (ventana desde 1-ago) ve solo su porción de agosto en adelante. Impacto vs el método por-entrega: `docs/private/comparativa_comisiones_prorrateo.md`. Construida por `build_comision_dia` en `build.py`; regenerar comparativa con `python scripts/build_comision_dia.py`.
14. **Periodos de contrato (`chra_mser`):** un contrato modificado/extendido republica sus cargos en periodos nuevos (`mser` 0, 1, 2…; siempre arranca en 0). **NO son duplicados** — la suma de todos los periodos es el total que reporta Sixt en `rntl_revenue_rental`. Cuentan completos como ingreso, pero **solo el periodo 0 comisiona**: la extensión arrastra el adicional automáticamente y el asesor no lo vuelve a vender. Expuesto como `cargo_periodo` en `vw_rentals_detail`. 34 contratos históricos son multi-periodo, 7 en 2026.
15. **operational.op_asesores:** tabla maestra manual de asesores (nombre + codigo_silver + codigo_hr). Se mantiene con INSERT/UPDATE manuales. El pipeline NO la toca. Requerida para mostrar nombres en Cargos Granular via LEFT JOIN por `codigo_silver = operador_handover_codigo`.
16. **Grano del vehiculo = SEGMENTO, no contrato (CRITICO, 2026-08-30):** un contrato puede cambiar de carro a mitad de renta. La verdad por vehiculo esta en `bronze.rent_shop_ra_fct_rental_vehicles_franchise` a grano `(rntl_mvnr, vhcl_int_num, rvnc_hser)` — 1 fila por tramo, con placa/fechas/odometro reales. El header (`ra_fct_rentals_vwt`) solo trae el PRIMER carro (`vhcl_int_num`) y el ultimo (`vhcl_int_num_current`). **La PK del upsert incluye `rvnc_hser`** (fix 2026-08-30, misma familia del bug de charges; sin el, una placa reasignada dos veces en un contrato pierde tramos). `gold_carro_dia` usa el timeline por segmento (v4). **Ante un hueco de ocupacion de un carro, consultar `ra_fct_rental_vehicles` por `vhcl_int_num`, NUNCA `ra_fct_rentals_vwt`** (es el punto ciego). La equivalencia Redshift=bronze=silver solo vale al grano contrato. **Pendiente:** `vw_rentals_full/resumen/detail` siguen usando la placa del header (1 fila/contrato); conviene agregar `placa_inicial`/`placa_final` o lista de placas. Ver `reference_datashare_completitud`.
...
| Presupuesto | `10_Presupuesto.py` | **solo trust_admin** | Presupuesto de ventas del próximo mes, **solo tarifa (cargo T)** — rentas de carro sin adicionales/coberturas/tax. Modelo bottom-up por (sede × categoría ACRISS): `flota × días × ocupación_esperada × RPD_tarifa`. Ocupación base = run-rate 3 meses × factor estacional (mes objetivo vs año anterior). Interactivo: editar ocupación esperada **por sede Y por categoría** (los factores se combinan multiplicativamente sobre la ocupación base) → recalcula en vivo (las columnas computadas se leen de `session_state["edited_rows"]` para que el presupuesto sea live). **Respeta el filtro de sedes del sidebar** (desde 2026-09-22): con UNA ciudad seleccionada, la tabla de categorías muestra y edita la ocupación de esa **(sede × ACRISS)** y se guarda con `dimension='sede_cat'`, clave `'GRUPO|ACRISS'`; un override por celda manda sobre el ajuste global de la categoría en cualquier alcance. Agrupa por CIUDAD: elegir cualquier sede de Medellín trae las dos. **Guardar** (a `operational.presupuesto_ocupacion`, PK `(mes, dimension, clave)` con dimension ∈ {sede,cat,sede_cat}) / **Volver a lo pre-calculado** (con una sede en alcance borra solo lo de esa sede). Escenarios ±10%; comparación vs el mismo mes del año anterior con delta. **La FLOTA es una foto del padrón activo al último día de `gold_carro_dia`, NO un conteo de la ventana de 3 meses** (fix 2026-09-22): antes, un carro que entraba después del cierre de la ventana (p.ej. el 24-sep con ventana jun–ago) contaba 0 y la flota solo crecía con ~2 meses de retraso, mientras que las bajas sí eran inmediatas (el roster de gold es el activo actual, así que un defleeteado desaparece retroactivamente). Ocupación y RPD siguen saliendo de la ventana de 3 meses — son tasas; lo que multiplica es la flota de hoy. Sede de cada placa = la de su **último día libre** (`rented_day=0`), no la modal de 30 días: en gold la sede solo se mueve con un **traslado** (`Internal Products`), así que un traslado entre ciudades mueve la flota del presupuesto el mismo día (la modal tardaba ~15 días; validado con NPM554 Medellín→Bucaramanga el 20-sep). Los días rentados se ignoran (ruido de one-way). Gate por `username == 'trust_admin'`. Lee `gold_carro_dia` (`tar_*`). |
