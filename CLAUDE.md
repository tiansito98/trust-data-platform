...
| Presupuesto | `10_Presupuesto.py` | **solo trust_admin** | Presupuesto de ventas del próximo mes, **solo tarifa (cargo T)** — rentas de carro sin adicionales/coberturas/tax. Modelo bottom-up por (sede × categoría ACRISS): `flota × días × ocupación_esperada × RPD_tarifa`. Ocupación base = run-rate 3 meses × factor estacional (mes objetivo vs año anterior). Interactivo: editar ocupación esperada **por sede Y por categoría** (los factores se combinan multiplicativamente sobre la ocupación base) → recalcula en vivo (las columnas computadas se leen de `session_state["edited_rows"]` para que el presupuesto sea live). **Guardar** (a `operational.presupuesto_ocupacion`, PK `(mes, dimension, clave)` con dimension ∈ {sede,cat}) / **Volver a lo pre-calculado** (borra los overrides del mes). Escenarios ±10%; comparación vs el mismo mes del año anterior con delta. Gate por `username == 'trust_admin'`. Lee `gold_carro_dia` (`tar_*`). |
